...
# --- Model and Prompts ---
try:
    # Using the official, working API identifier for the Flash model.
...
def select_by_llm_judge(user_query, responses_dict):
    ...
    **Your Task:**
    ...
    try:
        ...
        pattern = re.compile(r"WINNER:\s*(.*?)\s*REASON:\s*(.*?)\s*RESPONSE:(.*)", re.DOTALL | re.IGNORECASE)
        match = pattern.search(judgement_response)
        if match:
            winner_name, reason, final_response = match.groups()
            declaration = f"### 🧠 AntiSycophant  | Selected: **{winner_name.strip()}**\n<p style='font-size: 0.9em; color: #94A3B8;'><i>Judge's Reason: {reason.strip()}</i></p>"
            return final_response.strip(), declaration, responses_dict
        else:
            raise ValueError("LLM Judge did not follow the expected format.")
    except Exception as e:
        print(f"AntiSycophant failed: {e}. Returning a safe default.")
        declaration = "### ⚠️ System Error | Fallback to Safest Choice: **Fact-Checker**"
        response = responses_dict.get("Fact-Checker", "An error occurred during the final judging phase.")
...
def generate_and_select(query):
    ...
    for name, text in all_candidates_details.items():
        details_markdown += f"<div class='candidate-details-box'><h4>Candidate: {name}</h4><p class='full-text'>{strip_markdown(text)}</p></div>"
...
.gradio-container { max-width: 900px !important; margin: auto !important; padding-top: 20px !important; position: relative; }
.model-note { position: absolute; top: 20px; right: 20px; text-align: right; font-size: 0.75em; color: #CBD5E1; background-color: rgba(0, 30, 80, 0.6); padding: 5px 10px; border-radius: 8px; border: 1px solid rgba(255, 255, 255, 0.1); }
.model-note p { margin: 0; padding: 0; } /* Reset paragraph margin for the note */
...
theme = gr.themes.Base(font=[gr.themes.GoogleFont("Inter"), "ui-sans-serif", "system-ui", "sans-serif"]).set(
    body_background_fill="linear-gradient(135deg, #020617 0%, #0A192F 50%, #1E3A8A 100%)", body_text_color="#E2E8F0",
    button_primary_background_fill="linear-gradient(to right, #0EA5E9, #2563EB)", button_primary_text_color="#FFFFFF", button_primary_border_color_hover="#38BDF8"
)

with gr.Blocks(theme=theme, css=css, title="AntiSycophant") as demo:
    gr.Markdown(
        """<div class="model-note">
            <p>Tested on: <code>gemini-2.0-flash</code></p>
            <p>Results may differ on other models.</p>
        </div>"""
    )
    ...
    gr.Markdown(
        """<div style="text-align: center; padding: 10px 20px;">
            ...
            <p style="color: #94A3B8; font-size: 1.1em;">This app uses an LLM-as-a-Judge to select the most helpful and critical AI response with different prompting techniques.</p>
        </div>"""
    )
    ...
    with gr.Column(elem_classes="glassmorphism-block"):
        ...
        gr.Examples(
            ...
            ], inputs=input_textbox, label="Example Hypothetical Questions (Click to try)"
        )
